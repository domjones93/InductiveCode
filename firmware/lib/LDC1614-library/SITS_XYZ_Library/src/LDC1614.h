...
class LDC1614 {
public:
    LDC1614(TwoWire &iface, uint32_t clock, int address=0x2A);
    ...
    bool readAllChannels(InductanceData& inductance_data);  // TODO: Modify to work properly with varaible number of sensors
    bool rawReadAllChannels(uint16_t &status, RawData& raw_data); // TODO: Modify to work properly with varaible number of sensors
...
private:
    ...
    bool writeReg(uint8_t regAddr, uint16_t regValue);
    bool readValue(uint8_t regAddr, uint16_t &data);
    void logConfig(const String &message);
