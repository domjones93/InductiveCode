...
Adafruit_ST7789 tft = Adafruit_ST7789(TFT_CS, TFT_DC, TFT_RST); // Create ST7789 display object
...
float tare_x, tare_y, tare_z = 0;
float ind_x, ind_y, ind_z = 0;
...
void setup(void) {
  ...
  // init neopixel
  sensor = new LDC1614(Wire, 400000,0x2A);
  sensor->connect();
  ...
  RCount rcount;
  rcount.RCOUNT_CH0 = 0x100; //0x30BD;
  
  Offset offset;
  offset.OFFSET_CH0 = 0;

  SettleCount settlecount;
  settlecount.SETTLECOUNT_CH0 = 0x0100;
  ...
  tare_x = tare_data.inductance[0] + tare_data.inductance[1] - tare_data.inductance[2] - tare_data.inductance[3];
  tare_y = tare_data.inductance[0] - tare_data.inductance[1] - tare_data.inductance[2] + tare_data.inductance[3];
  tare_z = tare_data.inductance[0] + tare_data.inductance[1] + tare_data.inductance[2] + tare_data.inductance[3];
...
void loop() {
  ...
  ind_x = data.inductance[0] + data.inductance[1] - data.inductance[2] - data.inductance[3];
  ind_y = data.inductance[0] - data.inductance[1] - data.inductance[2] + data.inductance[3];
  ind_z = data.inductance[0] + data.inductance[1] + data.inductance[2] + data.inductance[3];
  ...
  for (int i = 1; i < 4; i++) {
    ...
    case 1:
      ...
      tft.print(values[i]); // Sensor x
      break;
    case 2:
      values[i] = String(pos_y, 4);
      tft.print(values[i]); // Sensor y
      break;
    case 3:
      values[i] = String(pos_z, 4);
      tft.print(values[i]); // Sensor z
      break;    
    default:
      break;
    }    
  ...
  // Serial.println("Sensor 7: " + String(pos_z, 6));
